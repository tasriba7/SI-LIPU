...
export const DEFAULT_CONFIG_DESA = {
  ...
  logo_url: null,
  kepala_desa_nama: "",
  kepala_desa_foto_url: null,
  kepala_desa_sambutan: "",
};
...
export async function getConfigDesa(supabase) {
  try {
    const { data, error } = await supabase
      .from("config_desa")
      .select(
        "id, jenis_wilayah, nama_desa, provinsi, kabupaten, kecamatan, alamat, foto_url, logo_url, kepala_desa_nama, kepala_desa_foto_url, kepala_desa_sambutan"
      )
      .eq("id", 1)
      .maybeSingle();
